...
if ($method === 'GET' && ($action === '' || $action === 'list')) {
  ...
  // Βάση SELECT – βάλε t.official_assign_date αν υπάρχει στη DB σου
  $sql = "
    SELECT
      t.id,
      t.status,
      t.created_at,
      t.updated_at,
      /* Αν το έχεις: t.official_assign_date, αλλιώς NULL για ασφάλεια */
  ...
  // Βασικές παράμετροι (ΜΟΝΑ pointers για να μην επαναχρησιμοποιούνται)
  $params = [
...
if ($method === 'POST' && $action === 'cancel_after_2y') {
  ...
  // Αν ΔΕΝ έχεις official_assign_date στη DB, βγάλε την TIMESTAMPDIFF συνθήκη
  $sql = "UPDATE theses
          SET status='canceled',
...
if ($method === 'POST' && ($action === 'accept_invitation' || $action === 'reject_invitation')) {
  ...
  // 1) update invitation (πρέπει να είναι προς εμένα & pending)
  $up = $pdo->prepare("
  ...
  // 2) αν accepted -> πρόσθεσε στο committee_members (αν δεν υπάρχει)
  if ($isAccept) {
...
if ($method === 'GET' && $action === 'show') {
  ...
  $sql = "
    SELECT
      ...
      /* Αν την έχεις βάλε t.official_assign_date */
      NULL AS official_assign_date,
      t.assigned_at,
